...
<div class="content">
	<section class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h1>Napravi klijenta</h1>
				</div>
				<div class="col-sm-6">
					<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="{{route('admin::index')}}">Početna</a></li>
						<li class="breadcrumb-item active">Napravi klijenta</li>
					</ol>
				</div>
			</div>
		</div><!-- /.container-fluid -->
	</section>
	<form id="client-form" class="p-3 mt-2" action="{{ route('admin::edit-client', [ $client->id ]) }}" method="post">
			...
			<div class="card-body">
				<div class="form-group form-inline ">
					...
					<select class="form-control" id="select-client-type" name="client_type">
						<option value="legal_entity" @if ($client->type == 'legal_entity') selected @endif>Pravno lice</option>
						<option value="individual" @if ($client->type == 'individual') selected @endif>Fizičko lice</option>
					</select>
				</div>
				<hr>
			...
			axios.post($(this).attr('action'), data)
				.then((response) => {
					...
					setTimeout(() => {
						...
						toastr['success']("Uspešno ste izmenili klijenta!");
					}, 400 - elapsed);
				})
				.catch((error) => {
					let elapsed = (Date.now() - start) / 1000.;
